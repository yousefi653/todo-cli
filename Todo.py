...
class Task:
    ...
    def add(self, task, deadline):
        ...
        if all_tasks:
            ...
            all_tasks.append({'id': last_id+1, 'task': task, 'deadline': deadline, 'complete': False})
        else:
            all_tasks.append({'id': 1, 'task': task, 'deadline': deadline, 'complete': False})
    ...
    def List(self, desc, complete, incomplete, deadline):
        ...
        if all_tasks:
            ...
            if deadline:
                ...
                distances = []
                for item in data:
                    temp = (jdatetime.datetime.strptime(item['deadline'], "%Y/%m/%d").date() - today).days
