...
# command add
@cli.command(
    help="(run add --help) You can add task => add (-t, --task) 'task1' (-d, --deadline) 1404/08/24"
)
@click.option("--task", "-t", prompt="> Enter task name", required=True)
@click.option(
    "--deadline", "-d", prompt="> Enter deadline for task(yyyy/mm/dd)", required=True
)
...
# command remove
@cli.command(
    help="(run remove --help) You can remvoe a task => remove --id 1| run remove --help"
)
@click.option(
    "--id",
    prompt="> Enter task's id",
    help="this option taken task's id for deleting the task.",
    type=int,
)
def remove(id):
    if id:
...
# command list
@cli.command(
    help="(run list --help) You can list of your task => list --options run | list --help"
)
@click.option("--desc", is_flag=True, help="this option descending tasks. list --desc")
@click.option(
    "--complete", is_flag=True, help="this option show complete tasks. list --complete"
)
@click.option(
    "--incomplete",
    is_flag=True,
    help="this option show incomplete tasks. list --incomplete",
)
@click.option(
    "--deadline",
    is_flag=True,
    help="this option show task by deadline. list --deadline",
)
        ...
        for item in data:
            ...
            mytable.add_row(
                [
                    ...
                    (
                        time_left
                        if type(time_left) is str
                        else f"{time_left} day is remains."
                    ),
                    item["complete"],
                ]
            )
...
# command complete
@cli.command(
    help="(run complete --help) You can change the complete value to True => complete --id 1 run | complete --help"
)
@click.option("--id", prompt=">Enter task's id: ", type=int)
def complete(id):
